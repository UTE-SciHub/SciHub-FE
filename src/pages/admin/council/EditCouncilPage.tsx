...
import { CouncilMemberRole, CouncilType, type CreateCouncilRequest } from "@/models/council"
import {
    Dialog,
    DialogContent,
    DialogDescription,
...
// Define the form schema for council creation/editing
...
export default function EditCouncilPage() {
    ...
    useEffect(() => {
        const fetchCouncilData = async () => {
            ...
            try {
                ...
                setOriginalCouncil(councilData)

                // Set form values
                form.reset({
                    name: councilData.name,
                    decisionNumber: councilData.decisionNumber,
                    establishmentDate: new Date(councilData.establishmentDate),
                    startDate: new Date(councilData.startDate),
                    endDate: new Date(councilData.endDate),
                    type: councilData.type,
                    notes: councilData.notes || "",
                })

                // Set council members
                if (councilData.councilMembers && councilData.councilMembers.length > 0) {
                    setCouncilMembers(
                        councilData.councilMembers.map((member: any) => ({
                            user: member.user,
                            role: member.role,
                        })),
                    )
                ...
                // Set selected topics
                if (councilData.topicCouncils && councilData.topicCouncils.length > 0) {
                    setSelectedTopics(councilData.topicCouncils.map((topicCouncil: any) => topicCouncil.topic))
                }
            } catch (error) {
    ...
    const onSubmit = async (data: z.infer<typeof councilFormSchema>) => {
        ...
        try {
            ...
            console.log("Submitting council data:", councilData);

            // Call service to update council
            const result = await CouncilService.update(Number(id), councilData)

            if (result.status === 200 && result.data.code === 1000) {
                toast({
                    title: "Thành công",
                    description: "Hội đồng đã được cập nhật thành công",
                    variant: "success",
                })

                // Navigate back to council detail
                navigate(`/admin/councils/${id}`)
            } else {
                toast({
                    title: "Lỗi",
                    description: result.data.message || "Có lỗi xảy ra khi cập nhật hội đồng. Vui lòng thử lại.",
                    variant: "error",
                })
            }
        } catch (error) {
            ...
            toast({
                title: "Lỗi",
                description: "Có lỗi xảy ra khi cập nhật hội đồng. Vui lòng thử lại.",
                variant: "error",
            })
        } finally {
    ...
    if (isLoading) {
        return <Loading onCancel={() => navigate("/admin/councils")} />
    }
        ...
        <div className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                <div className="flex items-center">
                    <Button
                        variant="ghost"
                        size="icon"
                        className="h-8 w-8 mr-2 text-gray-500 hover:text-gray-900"
                        onClick={() => navigate(`/admin/councils/${id}`)}
                    >
                        <ChevronLeft className="h-5 w-5" />
                        <span className="sr-only">Quay lại</span>
                    </Button>
                    <div>
                        <h1 className="text-2xl font-bold text-slate-900">Chỉnh sửa Hội đồng</h1>
                        <p className="text-slate-600 mt-1">Cập nhật thông tin hội đồng</p>
                    </div>
                </div>
            </div>
            ...
            <Card className="border-slate-200 shadow-sm">
                <CardHeader className="bg-slate-50 border-b border-slate-200">
                    ...
                    <CardDescription>Chỉnh sửa thông tin hội đồng</CardDescription>
                </CardHeader>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                        ...
                                        <div className="md:col-span-2">
                                            ...
                                            <div className="mt-2 flex items-center gap-4">
                                                ...
                                                ) : originalCouncil?.decisionFile ? (
                                                    <Badge variant="outline" className="bg-blue-50 text-blue-700 border-blue-200 px-3 py-1">
                                                        Đã có tệp quyết định
                                                    </Badge>
                                                ) : null}
                                            </div>
