...
const scoreSchema = Object.fromEntries(
    CRITERIA_DETAILS.map((criteria) => [
        criteria.id as keyof typeof formSchema.shape,
        z.number({
            required_error: `Điểm ${criteria.name} là bắt buộc`,
        }).min(criteria.minScore).max(criteria.maxScore),
    ])
);
...
const ReviewForm: React.FC<ReviewFormProps> = ({ topic, onNextStep, formData }) => {
    ...
    const form = useForm<z.infer<typeof formSchema>>({
        ...
        defaultValues: formData || Object.fromEntries(
            CRITERIA_DETAILS.map((criteria) => [criteria.id, criteria.minScore])
        ),
        mode: "onChange",
    });
    ...
    useEffect(() => {
        const sum = CRITERIA_DETAILS.reduce(
            (total, criteria) => total + (watchAllFields[criteria.id] || 0),
            0
        );
        setTotalScore(sum);
    }, [watchAllFields]);
    ...
    const handleSubmit = (data: z.infer<typeof formSchema>) => {
        ...
        const finalScore = CRITERIA_DETAILS.reduce(
            (total, criteria) => total + data[criteria.id],
            0
        );
                ...
                <div className="p-6">
                    ...
                    <Card className="mb-6">
                        <CardHeader className="pb-3 mb-3">
                            <CardTitle className="text-2xl font-bold">Người đánh giá</CardTitle>
